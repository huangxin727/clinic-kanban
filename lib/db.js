...
export async function tryAcquireAcceptLock(ticketId, requesterId, isAdmin) {
  const lockKey = `lock:accept:${ticketId}`
  const lockValue = JSON.stringify({ memberId: requesterId, isAdmin, ts: Date.now() })

  // 先尝试 SET NX（仅当 key 不存在时设置）
  const ok = await redis.set(lockKey, lockValue, { nx: true, ex: 5 })

  if (ok) {
    return { acquired: true, winner: true }
  }

  // key 已存在，说明有人在抢。读取当前锁持有者
  const existing = await redis.get(lockKey)
  if (!existing) {
    // 极端情况：锁刚过期，再抢一次
    const retry = await redis.set(lockKey, lockValue, { nx: true, ex: 5 })
    return { acquired: !!retry, winner: !!retry }
  }

  const holder = JSON.parse(existing)

  // 比较优先级：管理员 > 普通成员
  const currentIsAdmin = holder.isAdmin === true
  const requesterIsAdmin = isAdmin === true

  if (requesterIsAdmin && !currentIsAdmin) {
    // 请求者优先级更高（管理员抢占普通成员），直接覆盖锁
    await redis.set(lockKey, lockValue, { ex: 5 })
    return { acquired: true, winner: true }
  }

  // 优先级不够或相同，竞争失败
  return { acquired: true, winner: false }
}
